...
static const char *BUILTINS_IMPL =
    ...
    "template<typename T> static inline std::string _olrn_fmt_arg(const T& v) {\n"
    "    std::ostringstream _os; _os << v; return _os.str(); }\n"
    "static inline std::string _olrn_fmt_arg(const std::string& s) { return s; }\n"
    "static inline std::string _olrn_fmt_arg(const char* s) { return std::string(s); }\n"
    "static inline void _olrn_pf_runtime(std::ostream& out, const std::string& fmt,\n"
    ...
    "    std::vector<T> _data;\n"
    "    _OlrnList() = default;\n"
    "    void     add(T val)              { _data.push_back(std::move(val)); }\n"
    "    T        pop()                   { T x=std::move(_data.back()); _data.pop_back(); return x; }\n"
    "    void     remove(int64_t i)       { _data.erase(_data.begin()+(size_t)i); }\n"
    "    void     insert(int64_t i, T v)  { _data.insert(_data.begin()+(size_t)i, std::move(v)); }\n"
    "    void     clear()                 { _data.clear(); }\n"
    "    void     deinit()                { _data.clear(); _data.shrink_to_fit(); }\n"
    "    size_t   size()     const        { return _data.size(); }\n"
    "    size_t   capacity() const        { return _data.capacity(); }\n"
    "    void     reserve(size_t n)       { _data.reserve(n); }\n"
    "    T&       operator[](int64_t i)       { return _data[(size_t)i]; }\n"
    "    const T& operator[](int64_t i) const { return _data[(size_t)i]; }\n"
    ...
    "    _OlrnMap() = default;\n"
    "    void set(K key, V val) { _data[std::move(key)] = std::move(val); }\n"
    "    V    get(const K& key) const {\n"
    "        auto it = _data.find(key);\n"
    "        return it != _data.end() ? it->second : V{}; }\n"
    "    bool has(const K& key) const { return _data.count(key) > 0; }\n"
    "    void del(const K& key) { _data.erase(key); }\n"
    "    void clear()  { _data.clear(); }\n"
    "    void deinit() { _data.clear(); }\n"
    "    size_t  size() const { return _data.size(); }\n"
    "    void reserve(size_t n) { _data.reserve(n); }\n"
    "    _OlrnList<K> keys() const {\n"
    "        _OlrnList<K> r; r.reserve(_data.size());\n"
    "        for (auto& p : _data) r.add(p.first); return r; }\n"
    "    _OlrnList<V> vals() const {\n"
    "        _OlrnList<V> r; r.reserve(_data.size());\n"
    "        for (auto& p : _data) r.add(p.second); return r; }\n"
    "    auto begin()       { return _data.begin(); }\n"
    "    auto end()         { return _data.end(); }\n"
    "    auto begin() const { return _data.begin(); }\n"
    "    auto end()   const { return _data.end(); } };\n"
    "template<typename K, typename V>\n"
    "static inline _OlrnMap<K,V> _olrn_map_init(int64_t cap) {\n"
    "    _OlrnMap<K,V> m; m.reserve((size_t)cap); return m; }\n"
    "/* ── @set(T) hash set ────────────────────────────────────────────── */\n"
    "template<typename T>\n"
    "struct _OlrnSet {\n"
    "    std::unordered_set<T> _data;\n"
    "    _OlrnSet() = default;\n"
    "    void add(T val) { _data.insert(std::move(val)); }\n"
    "    bool has(const T& val) const { return _data.count(val) > 0; }\n"
    "    void del(const T& val) { _data.erase(val); }\n"
    "    void clear()  { _data.clear(); }\n"
    "    void deinit() { _data.clear(); }\n"
    "    size_t  size() const { return _data.size(); }\n"
    "    void reserve(size_t n) { _data.reserve(n); }\n"
    "    _OlrnList<T> items() const {\n"
    "        _OlrnList<T> r; r.reserve(_data.size());\n"
    "        for (auto& v : _data) r.add(v); return r; }\n"
    "    auto begin()       { return _data.begin(); }\n"
    "    auto end()         { return _data.end(); }\n"
    "    auto begin() const { return _data.begin(); }\n"
    "    auto end()   const { return _data.end(); } };\n"
    "template<typename T>\n"
    "static inline _OlrnSet<T> _olrn_set_init(int64_t cap) {\n"
    "    _OlrnSet<T> s; s.reserve((size_t)cap); return s; }\n"
    "template<typename T>\n"
    ...
    "    /* structural types — check before basic_string to avoid false match */\n"
    ...
    "    /* @map(K, V) */\n"
    "    if(s.size()>9&&s.compare(0,9,\"_OlrnMap<\")==0&&s.back()=='>') {\n"
    "        std::string inner=s.substr(9,s.size()-10);\n"
    "        int d=0; size_t c=inner.size();\n"
    "        for(size_t i=0;i<inner.size();i++) {\n"
    "            if(inner[i]=='<')d++; else if(inner[i]=='>')d--;\n"
    "            else if(inner[i]==','&&d==0){c=i;break;} }\n"
    "        std::string kk=inner.substr(0,c), vv=inner.substr(c+1);\n"
    "        if(!vv.empty()&&vv[0]==' ')vv=vv.substr(1);\n"
    "        return \"@map(\"+_olrn_map_type(kk)+\", \"+_olrn_map_type(vv)+\")\"; }\n"
    "    /* @set(T) */\n"
    "    if(s.size()>9&&s.compare(0,9,\"_OlrnSet<\")==0&&s.back()=='>') {\n"
    "        return \"@set(\"+_olrn_map_type(s.substr(9,s.size()-10))+\")\"; }\n"
    "    /* str — after structural types so containers-of-str don't false-match */\n"
...
static const char *STDLIB_IMPL =
    ...
    "static inline int64_t _olrn_str_index_of(const std::string& s, const std::string& p) {\n"
    "    auto pos = s.find(p); return pos==std::string::npos ? -1 : (int64_t)pos; }\n"
    "static inline std::string _olrn_str_substr(const std::string& s, int64_t start, int64_t len) {\n"
    "    if (start<0) start=0;\n"
    "    if (start>=(int64_t)s.size()) return {};\n"
    "    return s.substr((size_t)start, len<0 ? std::string::npos : (size_t)len); }\n"
    "static inline std::string _olrn_str_replace(const std::string& s, const std::string& f, const std::string& t) {\n"
    "    std::string r=s; size_t p=0;\n"
    "    while ((p=r.find(f,p))!=std::string::npos) { r.replace(p,f.size(),t); p+=t.size(); }\n"
    "    return r; }\n"
    "static inline std::vector<std::string> _olrn_str_split(const std::string& s, const std::string& d) {\n"
    "    std::vector<std::string> out; size_t p=0, n;\n"
    "    if (d.empty()) { for (char c:s) out.push_back(std::string(1,c)); return out; }\n"
    "    while ((n=s.find(d,p))!=std::string::npos) { out.push_back(s.substr(p,n-p)); p=n+d.size(); }\n"
    "    out.push_back(s.substr(p)); return out; }\n"
    "static inline std::string _olrn_str_join(const std::vector<std::string>& v, const std::string& sep) {\n"
    "    std::string r; for (size_t i=0;i<v.size();i++){if(i)r+=sep;r+=v[i];} return r; }\n"
    "static inline std::string _olrn_str_char_at(const std::string& s, int64_t i) {\n"
    "    if (i<0||i>=(int64_t)s.size()) return {};\n"
    "    return std::string(1, s[(size_t)i]); }\n"
    "static inline std::string _olrn_str_repeat(const std::string& s, int64_t n) {\n"
    "    std::string r; r.reserve(s.size()*(size_t)(n>0?n:0));\n"
    "    for (int64_t i=0;i<n;i++) r+=s; return r; }\n"
    "static inline std::string _olrn_str_trim_start(const std::string& s) {\n"
    "    size_t i=0; while(i<s.size()&&(unsigned char)s[i]<=' ')i++; return s.substr(i); }\n"
    "static inline std::string _olrn_str_trim_end(const std::string& s) {\n"
    "    size_t i=s.size(); while(i>0&&(unsigned char)s[i-1]<=' ')i--; return s.substr(0,i); }\n"
    "static inline std::string _olrn_str_pad_start(const std::string& s, int64_t w, const std::string& pad) {\n"
    "    if ((int64_t)s.size()>=w||pad.empty()) return s;\n"
    "    std::string r; while((int64_t)(r.size()+s.size())<w) r+=pad;\n"
    "    return r.substr(0,(size_t)(w-(int64_t)s.size()))+s; }\n"
    "static inline std::string _olrn_str_pad_end(const std::string& s, int64_t w, const std::string& pad) {\n"
    "    if ((int64_t)s.size()>=w||pad.empty()) return s;\n"
    "    std::string r=s; while((int64_t)r.size()<w) r+=pad;\n"
    "    return r.substr(0,(size_t)w); }\n"
    "static inline std::string _olrn_str_rev(const std::string& s) {\n"
    "    return std::string(s.rbegin(),s.rend()); }\n"
    "static inline int64_t _olrn_str_count(const std::string& s, const std::string& sub) {\n"
    "    if(sub.empty()) return 0; int64_t n=0; size_t p=0;\n"
    "    while((p=s.find(sub,p))!=std::string::npos){n++;p+=sub.size();} return n; }\n"
    "\n"
    "/* std.io */\n"
    ...
    "static inline void _olrn_io_writeln(int64_t h, const std::string& s) {\n"
    "    FILE* f = (FILE*)(uintptr_t)h;\n"
    "    if (f) { fputs(s.c_str(), f); fputc('\\n', f); } }\n"
    "/* FileRd — line-oriented buffered reader returned by io.file_rd() */\n"
    "#include <fstream>\n"
    "struct _OlrnFrdState {\n"
    "    std::ifstream f;\n"
    "    std::string   next;\n"
    "    bool          has_next = false;\n"
    "    bool          closed   = false;\n"
    "    void advance() { has_next = (bool)std::getline(f, next); }\n"
    "};\n"
    "struct FileRd {\n"
    "    std::shared_ptr<_OlrnFrdState> _s;\n"
    "    FileRd() = default;\n"
    "    explicit FileRd(const std::string& path) : _s(std::make_shared<_OlrnFrdState>()) {\n"
    "        std::string p = path;\n"
    "        if (!p.empty() && p[0] == '~') {\n"
    "            const char* h = std::getenv(\"HOME\");\n"
    "            if (h) p = std::string(h) + p.substr(1); }\n"
    "        _s->f.open(p);\n"
    "        if (_s->f.is_open()) _s->advance(); else _s->closed = true; }\n"
    "    bool has_ln() const { return _s && _s->has_next; }\n"
    "    _OlrnResult<std::string> read_ln() const {\n"
    "        if (!_s || !_s->has_next) return _OlrnResult<std::string>(_OlrnError{1, \"EOF\"});\n"
    "        std::string line = _s->next; _s->advance(); return _OlrnResult<std::string>(line); }\n"
    "    void close() const { if (_s && !_s->closed) { _s->f.close(); _s->closed = true; } }\n"
    "};\n"
    "static inline _OlrnResult<FileRd> _olrn_io_file_rd(const std::string& path) {\n"
    "    FileRd rd(path);\n"
    "    if (!rd._s || rd._s->closed)\n"
    "        return _OlrnResult<FileRd>(_OlrnError{1, \"cannot open file\"});\n"
    "    return _OlrnResult<FileRd>(std::move(rd)); }\n"
    "/* FileWr — line-oriented buffered writer returned by io.file_wr() */\n"
    "struct _OlrnFwrState { std::ofstream f; bool closed=false; };\n"
    "struct FileWr {\n"
    "    std::shared_ptr<_OlrnFwrState> _s;\n"
    "    FileWr() = default;\n"
    "    explicit FileWr(const std::string& path, bool append) : _s(std::make_shared<_OlrnFwrState>()) {\n"
    "        std::string p = path;\n"
    "        if (!p.empty() && p[0]=='~') { const char* h=std::getenv(\"HOME\"); if(h) p=std::string(h)+p.substr(1); }\n"
    "        _s->f.open(p, append ? std::ios::app : std::ios::out);\n"
    "        if (!_s->f.is_open()) _s->closed=true; }\n"
    "    void write_ln(const std::string& s) const { if (_s && !_s->closed) { _s->f << s << '\\n'; } }\n"
    "    void write(const std::string& s)    const { if (_s && !_s->closed) { _s->f << s; } }\n"
    "    void close() const { if (_s && !_s->closed) { _s->f.close(); _s->closed=true; } }\n"
    "};\n"
    "static inline _OlrnResult<FileWr> _olrn_io_file_wr(const std::string& path) {\n"
    "    FileWr wr(path, false);\n"
    "    if (!wr._s || wr._s->closed) return _OlrnResult<FileWr>(_OlrnError{1,\"cannot open file\"});\n"
    "    return _OlrnResult<FileWr>(std::move(wr)); }\n"
    "static inline _OlrnResult<FileWr> _olrn_io_file_wr_append(const std::string& path) {\n"
    "    FileWr wr(path, true);\n"
    "    if (!wr._s || wr._s->closed) return _OlrnResult<FileWr>(_OlrnError{1,\"cannot open file\"});\n"
    "    return _OlrnResult<FileWr>(std::move(wr)); }\n"
    "static inline _OlrnResult<std::string> _olrn_io_read_all(const std::string& path) {\n"
    "    std::string p=path;\n"
    "    if (!p.empty()&&p[0]=='~'){const char*h=std::getenv(\"HOME\");if(h)p=std::string(h)+p.substr(1);}\n"
    "    std::ifstream f(p); if (!f.is_open()) return _OlrnResult<std::string>(_OlrnError{1,\"cannot open file\"});\n"
    "    return _OlrnResult<std::string>(std::string(std::istreambuf_iterator<char>(f),{})); }\n"
    "static inline _OlrnResult<void> _olrn_io_write_all(const std::string& path, const std::string& data) {\n"
    "    std::string p=path;\n"
    "    if (!p.empty()&&p[0]=='~'){const char*h=std::getenv(\"HOME\");if(h)p=std::string(h)+p.substr(1);}\n"
    "    std::ofstream f(p); if (!f.is_open()) return _OlrnResult<void>(_OlrnError{1,\"cannot open file\"});\n"
    "    f<<data; return _OlrnResult<void>(); }\n"
    "\n"
    "/* std.env */\n"
    "static inline std::string _olrn_env_get(const std::string& name) {\n"
    "    const char* v = std::getenv(name.c_str());\n"
    "    return v ? std::string(v) : std::string(); }\n"
    "\n"
    "/* std.path */\n"
    "static inline std::string _olrn_path_join(const std::string& a, const std::string& b) {\n"
    "    return (std::filesystem::path(a)/b).string(); }\n"
    "static inline std::string _olrn_path_dir(const std::string& p) {\n"
    "    return std::filesystem::path(p).parent_path().string(); }\n"
    "static inline std::string _olrn_path_base(const std::string& p) {\n"
    "    return std::filesystem::path(p).filename().string(); }\n"
    "static inline std::string _olrn_path_stem(const std::string& p) {\n"
    "    return std::filesystem::path(p).stem().string(); }\n"
    "static inline std::string _olrn_path_ext(const std::string& p) {\n"
    "    return std::filesystem::path(p).extension().string(); }\n"
    "static inline std::string _olrn_path_abs(const std::string& p) {\n"
    "    std::error_code ec;\n"
    "    auto r = std::filesystem::absolute(p, ec);\n"
    "    return ec ? p : r.string(); }\n"
    "static inline bool _olrn_path_is_abs(const std::string& p) {\n"
